...
static void UpdateStartButtonPosition(const TaskbarInfo* taskBar, const WINDOWPOS* pPos)
{
	...
	else
	{
		...
		if (IsWin11() && (x == info.rcMonitor.left) && (buttonType == START_BUTTON_AERO || buttonType == START_BUTTON_CUSTOM) && !g_epTaskbar) {
			x += ScaleForDpi(taskBar->taskBar, 6);
			if (buttonType == START_BUTTON_CUSTOM) x += ScaleForDpi(taskBar->taskBar, 3);
		}
	}
...
public:
	...
	BOOLEAN PtInPeekBox(POINT point)
	{
		RECT rcBox;
		return GetPeekBox(&rcBox) && PtInRect(&rcBox, point);
	}
	BOOLEAN PtInPeekBox()
	{
		RECT rcBox;
...
private:
	...
	{
		if (!trayWnd)
			trayWnd = FindWindowEx(g_TaskBar, NULL, L"TrayNotifyWnd", NULL);
		if (GetWindowRect(trayWnd, RectBox))
		{
			RectBox->left = RectBox->right - ScaleForDpi(trayWnd, 12);
			RectBox->right += 1, RectBox->bottom += 1;
			return TRUE;
		}
		return FALSE;
	}
